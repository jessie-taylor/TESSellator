...
# Version for non KIC input, with any ID
def any_TICer(MAIN_ID: str):
  """ 
  Function which uses the Simbad database to obtain
  the TIC identifier of a star using any format identifier
  
  Parameters
  ----------
  MAIN_ID : str
      The ID of the star, as you would search Simbad with.
  ...
  """
  # Obtain all associated IDs with the given ID#
  ids = Simbad.query_objectids((MAIN_ID))
  # Convert to dataframe
  dfids = pd.DataFrame()
  # Still converting to dataframe
  # But error handling in case ID not found in Simbad
  try:
    ...
    warnings.warn("ID not found in Simbad database " + MAIN_ID
                                                     + "returning None")
    return None 
  # Properly decode
  dfids["ID"] = dfids["ID"].str.decode("utf-8")
  ...
  try:
    ...
    warnings.warn("No TIC found for input ID " + MAIN_ID
                                               + " returning None")
    return None
